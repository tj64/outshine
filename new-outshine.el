...
;; FIXME good idea?
;; Making outshine more powerful by requiring Org-mode
;; (require 'outline)
(require 'org)
(require 'easymenu)
...
;;;###autoload
(define-derived-mode outshine-mode org-mode "Outshine"
  "Set major mode for editing outshines with selective display.
...
Turning on outline mode calls the value of `org-mode-hook' and then of
`outshine-mode-hook', if they are non-nil."
...
(defcustom outshine-minor-mode-prefix "\M-#"
  "Prefix key to use for Outshine commands in Outshine minor mode.
The value of this variable is checked as part of loading Outshine mode.
...
(define-minor-mode outshine-minor-mode
  "Toggle Outshine minor mode.
With a prefix argument ARG, enable Outshine minor mode if ARG is
positive, and disable it otherwise.  If called from Lisp, enable
the mode if ARG is omitted or nil.

See the command `outshine-mode' for more information on this mode."
  nil " Outl" (list (cons [menu-bar] outshine-minor-mode-menu-bar-map)
		    (cons outshine-minor-mode-prefix outshine-mode-prefix-map))
  :group 'outlines
  (if outshine-minor-mode
      (progn
	;; Turn off this mode if we change major modes.
	(add-hook 'change-major-mode-hook
		  (lambda () (outshine-minor-mode -1))
		  nil t)
	(set (make-local-variable 'line-move-ignore-invisible) t)
	;; Cause use of ellipses for invisible text.
	(add-to-invisibility-spec '(outshine . t)))
    (setq line-move-ignore-invisible nil)
    ;; Cause use of ellipses for invisible text.
    (remove-from-invisibility-spec '(outshine . t))
    ;; When turning off outshine mode, get rid of any outshine hiding.
    (show-all)))
